...
function startPractice() {
    ...
    // Show/hide reveal button based on mode
    if (flashcardSettings.mode === 'manual') {
...
function nextFlashcard() {
    ...
    console.log('Flashcard settings:', flashcardSettings);
    console.log('Available chords:', Object.keys(instrument.chords));

    // Generate flashcard based on practice type
    if (flashcardSettings.practiceType === 'notes') {
        currentFlashcard = generateNoteFlashcard(instrument);
        console.log('Generated note flashcard:', currentFlashcard);
    } else if (flashcardSettings.practiceType === 'chords') {
        currentFlashcard = generateChordFlashcard(instrument);
        console.log('Generated chord flashcard:', currentFlashcard);

        // If no chords available, fall back to notes
        if (!currentFlashcard) {
            console.log('No chords available, falling back to notes');
            currentFlashcard = generateNoteFlashcard(instrument);
        }
    } else if (flashcardSettings.practiceType === 'both') {
        // Randomly choose between notes and chords
        const random = Math.random();
        if (random < 0.7) { // 70% chance for notes, 30% for chords
            currentFlashcard = generateNoteFlashcard(instrument);
        } else {
            currentFlashcard = generateChordFlashcard(instrument);
            // If no chords available, use notes instead
            if (!currentFlashcard) {
                currentFlashcard = generateNoteFlashcard(instrument);
            }
        }
    ...
    if (flashcardSettings.mode === 'timed') {
        ...
        }, flashcardSettings.timeLimit * 1000);
    }
